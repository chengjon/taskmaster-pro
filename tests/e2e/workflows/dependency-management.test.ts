...
describe('Dependency Management E2E', () => {
	...
	describe('add-dependency command', () => {
		...
		it('should add a dependency between tasks', async () => {
			const { stdout, stderr } = await env.runCLI(
				'add-dependency --id=1 --depends-on=3'
			);

			assertCommandSuccess(stderr);
			assertOutputContains(stdout, 'dependency');
			assertOutputContains(stdout, 'added');

			// Verify dependency was added
			await assertDependencyExists(env, '1', '3');
		});

		it('should handle multiple dependencies', async () => {
			// Task 2 already depends on 1
			await env.runCLI('add-dependency --id=2 --depends-on=3');

			// Task 2 should now depend on both 1 and 3
	...
	describe('dependency visualization', () => {
		...
		it('should show task dependencies in show command', async () => {
			...
			assertOutputContains(stdout, 'depends');
			assertOutputContains(stdout, '1');
		});

		it('should show dependent tasks in show command', async () => {
			// Task 1 is depended on by Task 2
			const { stdout } = await env.runCLI('show 1');

			// Should indicate this task is blocking others
			expect(stdout.toLowerCase()).toMatch(/blocked by|dependent|required/);
		});
	});
	...
	describe('error recovery', () => {
		it('should recover from dependency validation failures', async () => {
			const tasksData = createTasksData(simpleTasks);
			await env.writeFile(
				'.taskmaster/tasks/tasks.json',
				JSON.stringify(tasksData, null, 2)
