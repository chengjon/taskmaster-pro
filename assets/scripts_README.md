...
1.  **`.taskmaster/config.json` File (Project Root - Primary)**
    ...
    - Managed using the `TAMP models --setup` command or the `models` MCP tool.
...
**Important:** Settings like `MODEL`, `MAX_TOKENS`, `TEMPERATURE`, `TASKMASTER_LOG_LEVEL`, etc., are **no longer set via `.env`**. Use `TAMP models --setup` instead.
...
2. **CLI Commands**
   ...
   Run `TAMP --help` or `node scripts/dev.js --help` to see detailed usage information.
...
TAMP list --status=pending --with-subtasks
```
...
TAMP update --from=4 --prompt="Refactor tasks from ID 4 onward to use Express instead of Fastify"

# Update all tasks (default from=1)
TAMP update --prompt="Add authentication to all relevant tasks"

# Specify a different tasks file
TAMP update --file=custom-tasks.json --from=5 --prompt="Change database from MongoDB to PostgreSQL"
```
...
TAMP set-status --id=4 --status=pending

# Mark a specific subtask as done
TAMP set-status --id=3.1 --status=done
...
TAMP expand --id=3 --num=5

# Expand a task with additional context
TAMP expand --id=3 --prompt="Focus on security aspects"
...
- To use the research capabilities (e.g., `expand --research`), ensure you have:
  1.  Configured a model for the `research` role using `TAMP models --setup` (Perplexity models are recommended).
...
# Specify a different tasks file
TAMP validate-dependencies --file=custom-tasks.json
```
...
# Specify a different tasks file
TAMP fix-dependencies --file=custom-tasks.json
```
...
TAMP analyze-complexity --output=custom-report.json
...
TAMP expand --id=8 --num=5 --prompt="Custom prompt"
```
  ...
  "complexityAnalysis": [
    {
      ...
      "expansionCommand": "TAMP expand --id=8 --num=6 --prompt=\"Create subtasks...\" --research"
    }
...
# Specify a different tasks file
TAMP next --file=custom-tasks.json
```
...
# Specify a different tasks file
TAMP show 3 --file=custom-tasks.json
```
