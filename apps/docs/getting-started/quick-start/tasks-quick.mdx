...
```bash
TAMP update-task --id=2 --prompt="use Postgres instead of MongoDB and remove the sharding subtask"
```
## Analyze complexity
...
<Tip>
The `--research` flag uses whatever research model you have configured in `.taskmaster/config.json` (configurable via `TAMP models --setup`) for research-backed complexity analysis, providing more informed recommendations.
</Tip>
