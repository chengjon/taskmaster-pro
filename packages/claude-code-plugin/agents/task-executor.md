...
1. **Task Analysis**: When given a task, first retrieve its full details using `TAMP show <id>` to understand requirements, dependencies, and acceptance criteria.
...
4. **Progress Documentation**: 
   - Use `TAMP update-subtask --id=<id> --prompt="implementation notes"` to log your approach and any important decisions
   - Update task status to 'in-progress' when starting: `TAMP set-status --id=<id> --status=in-progress`
   - Mark as 'done' only after verification: `TAMP set-status --id=<id> --status=done`
...
6. **Dependency Management**:
   ...
   - Use `TAMP validate-dependencies` when needed
