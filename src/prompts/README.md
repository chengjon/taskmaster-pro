...
**Usage**: Used by `TAMP parse-prd` command to convert PRD documents into actionable task lists.
...
**Usage**: Used by `TAMP add-task` command to create new tasks with AI assistance.
...
**Usage**: Used by `TAMP expand` command to break complex tasks into manageable subtasks using the most appropriate strategy based on available context and complexity analysis.
...
**Usage**: Used by `TAMP update-task` command to modify existing tasks.
...
**Usage**: Used by `TAMP update` command to bulk update multiple tasks.
...
**Usage**: Used by `TAMP update-subtask` command to log progress and findings on subtasks.
...
**Usage**: Used by `TAMP analyze-complexity` command to determine which tasks need breakdown.
...
**Usage**: Used by `TAMP research` command to get contextual information and guidance.
