...
1.  **`.taskmaster/config.json` File (Recommended - New Structure)**
    ...
    - **Location:** This file is created in the `.taskmaster/` directory when you run the `TAMP models --setup` interactive setup or initialize a new project with `TAMP init`.
    - **Migration:** Existing projects with `.taskmasterconfig` in the root will continue to work, but should be migrated to the new structure using `TAMP migrate`.
    - **Management:** Use the `TAMP models --setup` command (or `models` MCP tool) to interactively create and manage this file. You can also set specific models directly using `TAMP models --set-<role>=<model_id>`, adding `--ollama` or `--openrouter` flags for custom models. Manual editing is possible but not recommended unless you understand the structure.
    - **Example Structure:**
      ```json
      {
        "models": {
...
2.  **Legacy `.taskmasterconfig` File (Backward Compatibility)**
    ...
    - **Migration:** Use `TAMP migrate` to move this to `.taskmaster/config.json`.
...
- **Manual Tag Creation**: Use `TAMP add-tag --from-branch` to create a tag based on your current git branch name
...
- If Task Master reports errors about missing configuration or cannot find the config file, run `TAMP models --setup` in your project root to create or repair the file.
- For new projects, config will be created at `.taskmaster/config.json`. For legacy projects, you may want to use `TAMP migrate` to move to the new structure.
...
7. **Setup Commands**:
   ...
   TAMP models set-main --provider mcp --model claude-3-5-sonnet-20241022

   # Set MCP provider for research role
   TAMP models set-research --provider mcp --model claude-3-opus-20240229
...
TAMP rules add cursor
TAMP rules add roo
TAMP rules add windsurf
TAMP rules add vscode
```
