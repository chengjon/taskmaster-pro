...
TAMP add-task --prompt="Implement user authentication with JWT" --research
```
...
TAMP parse-prd my-feature.txt --num-tasks 12
```
...
TAMP expand --id=1.2 --research --force
```
...
TAMP parse-prd requirements.txt
...
TAMP parse-prd feature-prd.txt --num-tasks 8
...
TAMP add-task --prompt="Build REST API endpoint"

# Research operations use Perplexity
TAMP analyze-complexity --research

# Fallback to GPT-5 if needed
TAMP expand --id=3.2 --force
```
  ...
  analyze:
    ...
    steps:
      ...
      - name: Install Task Master
        run: npm install -g taskmaster-pro
      ...
      - name: Analyze Complexity
        run: TAMP analyze-complexity --research
...
TAMP models --set-fallback claude-3-5-sonnet
```
...
TAMP update-subtask --id=2.3 --prompt="Implemented authentication middleware"

# Mark complete
TAMP set-status --id=2.3 --status=done
...
# Generate tasks
TAMP parse-prd new-feature.txt --num-tasks 10

# Analyze and expand
TAMP analyze-complexity --research
TAMP expand --all --research --force
...
TAMP parse-prd sprint-requirements.txt
