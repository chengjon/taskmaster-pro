...
# Move multiple tasks
TAMP move --from=5,6,7 --to=10,11,12
```
...
# Move multiple tasks
TAMP move --from=5,6,7 --from-tag=backlog --to-tag=done
```
...
```bash
TAMP move --from=5 --from-tag=backlog --to-tag=in-progress --with-dependencies
```
...
Resolution options:
  1. Move with dependencies: TAMP move --from=5,6 --from-tag=backlog --to-tag=in-progress --with-dependencies
  2. Break dependencies: TAMP move --from=5,6 --from-tag=backlog --to-tag=in-progress --ignore-dependencies
  3. Validate and fix dependencies: TAMP validate-dependencies && TAMP fix-dependencies
  4. Move dependencies first: TAMP move --from=2,3 --from-tag=backlog --to-tag=in-progress
...
Resolution options:
  1. Promote subtask to full task: TAMP remove-subtask --id=5.2 --convert
  2. Then move the promoted task: TAMP move --from=5 --from-tag=backlog --to-tag=in-progress
  3. Or move the parent task with all subtasks: TAMP move --from=5 --from-tag=backlog --to-tag=in-progress --with-dependencies
```
...
Resolution options:
  ...
  2. Use within-tag move: TAMP move --from=<id> --to=<id> --tag=backlog
  3. Check available tags: TAMP tags
```
...
TAMP move --from=5,6,7 --from-tag=backlog --to-tag=in-progress --with-dependencies

# Move multiple tasks, breaking dependencies
TAMP move --from=5,6,7 --from-tag=backlog --to-tag=in-progress --ignore-dependencies
```
...
TAMP move --from=5 --from-tag=backlog --to-tag=new-feature
```
...
TAMP move --from=5 --to-tag=in-progress
```
...
1. **"Source tag not found"**: Check available tags with `TAMP tags`
2. **"Task not found"**: Verify task IDs with `TAMP list`
...
TAMP move --from=5 --from-tag=backlog --to-tag=done --ignore-dependencies
```
