...
Taskmaster is also available via CLI commands, by installing with `npm install taskmaster-pro@latest` in a terminal. Running `TAMP help` will show all available commands, which offer a 1:1 experience with the MCP server. As the AI agent, you should refer to the system prompts and rules provided to you to identify Taskmaster-specific rules that help you understand how and when to use it.
