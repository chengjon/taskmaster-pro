...
```javascript
// In your .taskmaster/config.json or via TAMP models command
{
  "models": {
...
TAMP add-task --prompt="Implement user authentication system" --research
TAMP expand --id=1 --research
TAMP update-task --id=1.1 --prompt="Add JWT token validation"
```
...
TAMP models --set-main claude-code:sonnet
...
- Check model availability: `TAMP models --list`
...
3. **Test Task Master integration**:
   ...
   ```bash
   TAMP models --test claude-code:sonnet
