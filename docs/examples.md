...
(Agent runs: `TAMP move --from=5.2 --to=7.3`)

```
Task 8 should actually be a subtask of task 4. Can you reorganize this?
```

(Agent runs: `TAMP move --from=8 --to=4.1`)
...
(Agent runs: `TAMP expand --id=5`)

```
Please break down task 5 using research-backed generation.
```

(Agent runs: `TAMP expand --id=5 --research`)
...
(Agent runs: `TAMP update-task --id=15 --prompt="Update based on React Query v5 changes" --research`)
...
(Agent runs: `TAMP add-task --prompt="Implement user profile image uploads using Cloudinary" --research`)
...
(Agent runs: `TAMP research "Latest best practices for JWT authentication in Node.js"`)
...
(Agent runs: `TAMP research "API optimization best practices" --id=15 --files=src/api.js`)
...
(Agent runs: `TAMP research "React Query v5 patterns and breaking changes" --id=8`)
...
1. `TAMP research "Latest Express.js security recommendations" --id=12`
2. `TAMP update-subtask --id=12.3 --prompt="Updated with latest security findings: [research results]"`)
...
(Agent runs: `TAMP research "Common WebSocket implementation problems and solutions" --id=20 --files=src/websocket.js`)
...
(Agent runs: `TAMP research "Redis vs Memcached 2024 comparison for session caching" --tree`)
...
(Agent runs: `TAMP add-tag --from-branch`)
...
(Agent runs: `TAMP add-tag api-v2 --description="API v2 redesign tasks"`)
...
(Agent runs: `TAMP use-tag testing`)
...
(Agent runs: `TAMP add-tag hotfix --copy-from-current --description="Urgent hotfix tasks"`)
...
(Agent runs: `TAMP tags --show-metadata`)
...
(Agent runs: `TAMP delete-tag user-auth`)
...
(Agent runs: `TAMP use-tag api-v2` then `TAMP list`)
...
2. `TAMP add-tag --from-branch --description="Payment system implementation"`
3. `TAMP list` to show tasks in the new context)
...
1. `TAMP add-tag auth --description="Authentication feature tasks"`
2. `TAMP add-tag payments --description="Payment system tasks"`
3. `TAMP use-tag auth` to start with authentication work)
