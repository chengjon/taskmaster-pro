...
1.  **`.taskmasterconfig` File (Project Root)**
    ...
    - **Created and managed using `TAMP models --setup` CLI command or the `models` MCP tool.**
...
**Important:** Settings like model choices, max tokens, temperature, and log level are **no longer configured via environment variables.** Use the `TAMP models` command or tool.
...
```
Please use the TAMP parse-prd command to generate tasks from my PRD. The PRD is located at scripts/prd.txt.
```

The agent will execute:

```bash
TAMP parse-prd scripts/prd.txt
```
...
```bash
TAMP update --from=4 --prompt="Now we are using Express instead of Fastify."
```
