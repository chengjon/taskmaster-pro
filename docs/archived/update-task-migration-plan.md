...
TAMP update-task --id=3 --prompt="changes"

# Update single subtask (replaces update-subtask)
TAMP update-task --id=3.2 --prompt="implementation notes"

# Update multiple tasks from ID onwards (replaces update --from)
TAMP update-task --from=3 --prompt="changes"
```
...
TAMP update --from=3 --prompt="..."    # Still works, calls update-task
TAMP update-subtask --id=3.2 --prompt="..."  # Still works, calls update-task
```
