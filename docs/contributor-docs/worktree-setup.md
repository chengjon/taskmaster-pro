...
# In Claude session
> Use TAMP to get the next task and complete it
```
...
```bash
# In any worktree
TAMP list        # Same tasks
TAMP next        # Same task queue
TAMP show 1.2    # Same task data
...
cd ../claude-task-master-worktrees/auto-main/
TAMP add-tag --name=claude-auto
TAMP use-tag --name=claude-auto
TAMP list  # Shows claude-auto tasks only
```
