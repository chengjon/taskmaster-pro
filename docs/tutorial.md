...
**IMPORTANT:** An API key is _required_ for each AI provider you plan on using. Run the `TAMP models` command to see your selected models and the status of your API keys across .env and mcp.json
...
```
Please use the TAMP parse-prd command to generate tasks from my PRD. The PRD is located at .taskmaster/docs/prd.txt.
```
...
- Run `TAMP list` to see all tasks
- Run `TAMP next` to determine the next task to work on
- Run `TAMP show 1,3,5` to display multiple tasks with interactive options
...
- Run `TAMP show 5,7,9` to display a compact summary table
...
```bash
TAMP set-status --id=3 --status=done
```
...
```bash
TAMP update --from=4 --prompt="Now we are using MongoDB instead of PostgreSQL."

# OR, if research is needed to find best practices for MongoDB:
TAMP update --from=4 --prompt="Update to use MongoDB, researching best practices" --research
```
...
TAMP move --from=10 --to=16
TAMP move --from=11 --to=17
TAMP move --from=12 --to=18
```
...
```bash
TAMP expand --id=5 --prompt="Focus on security aspects"
```
...
```bash
TAMP research "Latest JWT security recommendations 2024" --id=5
```
...
```bash
TAMP research "React Query v5 migration strategies" --files=src/api.js,src/hooks.js
```
...
1. Run research: `TAMP research "Node.js performance optimization 2024" --id=12`
2. Update the task: `TAMP update-subtask --id=12.2 --prompt="Updated with latest performance findings: [research results]"`
...
```bash
TAMP add-tag user-auth --description="User authentication feature tasks"
```
...
```bash
TAMP add-tag testing --copy-from-current --description="QA and testing tasks"
```
...
2. **Create matching tag**: Ask agent to run `TAMP add-tag --from-branch`
3. **Work in isolated context**: All task operations work within the new tag
4. **Switch contexts as needed**: Use `TAMP use-tag <name>` to switch between different work streams
5. **Merge and cleanup**: After merging the branch, optionally delete the tag with `TAMP delete-tag <name>`
